...
// Attack enemies in range of towers
function towerAttackEnemies() {
    const TOWER_RANGE = 15; // Adjust as needed
    const TOWER_DAMAGE = 2; // Damage per tick
    const ATTACK_INTERVAL = 500; // ms between attacks

    // Use a timestamp to control attack rate
    if (!towerAttackEnemies.lastAttackTime) towerAttackEnemies.lastAttackTime = 0;
    const now = Date.now();
    if (now - towerAttackEnemies.lastAttackTime < ATTACK_INTERVAL) return;
    towerAttackEnemies.lastAttackTime = now;

    buildings.forEach((building) => {
        if (building.userData.type === "tower") {
            enemies.forEach((enemy) => {
                const dist = building.position.distanceTo(enemy.position);
                if (dist <= TOWER_RANGE) {
                    enemy.userData.health -= TOWER_DAMAGE;
                    // Optional: Visual effect or log
                    // console.log(`Tower hit enemy! Enemy health: ${enemy.userData.health}`);
                    if (enemy.userData.health <= 0) {
                        scene.remove(enemy);
